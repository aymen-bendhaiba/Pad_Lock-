...
type StoredReportJob = ReportJob & { saved?: true };

const GENERATED_REPORT_PREFIX = "generated-";
const STORED_REPORTS_KEY = "padlock-generated-reports-v1";
...
function isStoredReport(value: unknown): value is StoredReportJob { const record = asRecord(value); if (!record) return false; const definition = asRecord(record.definition); const filters = asRecord(record.filters); return typeof record.id === "string" && record.id.startsWith(GENERATED_REPORT_PREFIX) && typeof record.name === "string" && typeof record.type === "string" && typeof record.date === "string" && typeof record.time === "string" && typeof record.size === "string" && ["Ready", "Processing", "Error"].includes(String(record.status)) && Boolean(definition?.key) && Boolean(filters?.kind); }
function readStoredReports(): StoredReportJob[] { if (typeof window === "undefined") return []; try { const parsed = JSON.parse(window.localStorage.getItem(STORED_REPORTS_KEY) ?? "[]") as unknown; return Array.isArray(parsed) ? parsed.filter(isStoredReport).map(report => ({ ...report, saved: true })) : []; } catch { return []; } }
function writeStoredReports(reports: StoredReportJob[]) { if (typeof window === "undefined") return; try { window.localStorage.setItem(STORED_REPORTS_KEY, JSON.stringify(reports.slice(0, 50))); } catch { /* Storage can fail in private mode or when full. */ } }
function mergeReports(saved: StoredReportJob[], generated: ReportJob[], summaries: ReportJob[]) { const seen = new Set<string>(); return [...saved, ...generated, ...summaries].filter(report => { if (seen.has(report.id)) return false; seen.add(report.id); return true; }); }
...
export function ReportsPanel() {
  ...
  const [savedReports, setSavedReports] = useState<StoredReportJob[]>(() => readStoredReports());
  ...
  useEffect(() => { const stored = readStoredReports(); setSavedReports(stored); setReports(current => mergeReports(stored, current.filter(item => item.id.startsWith(GENERATED_REPORT_PREFIX)), current.filter(item => !item.id.startsWith(GENERATED_REPORT_PREFIX)))); }, []);
  ...
  function saveGeneratedReport(report: StoredReportJob) { setSavedReports(current => { const next = [report, ...current.filter(item => item.id !== report.id)].slice(0, 50); writeStoredReports(next); return next; }); }
  function removeGeneratedReport(id: string) { setSavedReports(current => { const next = current.filter(item => item.id !== id); writeStoredReports(next); return next; }); }
  async function loadDefaultReports(nextTerminalId = lockFilter === "All" ? "" : lockFilter) { setLoading(true); setMessageTone("info"); setMessage("Chargement de l'historique des rapports..."); const baseFilters = { ...defaultFilters(), terminalId: nextTerminalId }; const baseJobs = reportDefinitions.map(definition => jobFrom(definition, { ...baseFilters, kind: definition.key })); setReports(current => mergeReports(savedReports, current.filter(item => item.id.startsWith(GENERATED_REPORT_PREFIX)), baseJobs)); try { const summaryPayload = await fetchReportsSummary(baseFilters); const jobs = reportDefinitions.map(definition => { const filters = { ...baseFilters, kind: definition.key }; const summary = summaryPayload.reports?.[definition.key] ?? {}; const response: ReportResponse = { range: summaryPayload.range, filters: summaryPayload.filters, summary, rows: [], timeline: [] }; return jobFrom(definition, filters, response); }); setReports(current => mergeReports(savedReports, current.filter(item => item.id.startsWith(GENERATED_REPORT_PREFIX)), jobs)); setMessage("Historique des rapports"); } catch (error) { const errorJobs = baseJobs.map(job => ({ ...job, status: "Error" as ReportStatut, error: userFriendlyError(error, "Echec"), size: "--" })); setReports(current => mergeReports(savedReports, current.filter(item => item.id.startsWith(GENERATED_REPORT_PREFIX)), errorJobs)); setMessageTone("error"); setMessage(userFriendlyError(error, "Impossible de charger l'historique des rapports.")); } finally { setLoading(false); } }
  async function generateReport(filters: ReportFilters) { const definition = reportDefinitions.find(d => d.key === filters.kind) ?? reportDefinitions[0]; setLoading(true); setMessageTone("info"); setMessage("Generation du rapport..."); const pending = { ...jobFrom(definition, filters), id: GENERATED_REPORT_PREFIX + Date.now() + "-" + Math.random().toString(36).slice(2) }; setReports(current => [pending, ...current]); try { const response = await fetchReport(filters); const time = timestampParts(); const ready: StoredReportJob = { ...pending, status: "Ready", response, size: formatSize(response), date: time.date, time: time.time, saved: true }; saveGeneratedReport(ready); setReports(current => current.map(item => item.id === pending.id ? ready : item)); setShowModal(false); setMessage("Rapport genere et enregistre."); } catch (error) { setReports(current => current.map(item => item.id === pending.id ? { ...item, status: "Error", error: userFriendlyError(error, "Echec"), size: "--" } : item)); setMessageTone("error"); setMessage(userFriendlyError(error, "Impossible de generer le rapport.")); } finally { setLoading(false); } }
  function deleteReport(id: string) { removeGeneratedReport(id); setReports(current => current.filter(item => item.id !== id)); setSelectedIds(current => current.filter(item => item !== id)); setOpenMenuId(null); }
