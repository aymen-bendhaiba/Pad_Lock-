...
function normalizeReportResponse(kind: ReportKind, response: ReportResponse) {
  if (kind !== "geofences") return response;
  const rows = (response.rows ?? []).map(row => {
    const record = rowRecord(row);
    return Object.fromEntries(Object.entries(record).filter(([key]) => !["id", "rules", "geoBoundaryId"].includes(key)));
  });
...
function batteryTimestamp(record: ApiRecord, index: number) { return textValue(record.bucket, record.date, record.day, record.timestamp, record.createdAt, record.occurredAt, record.updatedAt, record.recordedAt, record.lastRecordedAt, record.time) ?? "Point " + (index + 1); }
function batteryPercentage(record: ApiRecord) { const direct = numberValue(record.latest, record.latestPercentage, record.average, record.averagePercentage, record.avgPercentage, record.percentage, record.batteryPercentage, record.batteryPercent, record.batteryLevel, record.level, record.value, record.minimum, record.minimumPercentage, record.maximum, record.maximumPercentage, record.battery); return direct === undefined ? undefined : Math.max(0, Math.min(100, direct)); }
function batteryChartSource(response?: ReportResponse) { const timeline = response?.timeline ?? []; return timeline.length ? timeline : response?.rows ?? []; }
...
export function ReportsPanel() {
  ...
  async function loadReportDetails(report: ReportJob) { const normalizedReport = report.response ? { ...report, response: normalizeReportResponse(report.definition.key, report.response) } : report; if (hasDetailedReport(normalizedReport)) return normalizedReport; setLoading(true); setMessageTone("info"); setMessage("Chargement des details du rapport..."); try { const response = normalizeReportResponse(report.definition.key, await fetchReport(report.filters)); const detailed = { ...normalizedReport, status: "Ready" as ReportStatut, response, size: formatSize(response), error: undefined }; if (loadCreatedReports().some(item => item.id === report.id)) upsertCreatedReport(detailed); setReports(current => current.map(item => item.id === report.id ? detailed : item)); return detailed; } finally { setLoading(false); } }
